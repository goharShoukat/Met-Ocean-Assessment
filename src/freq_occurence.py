...
def frequency_occurence_wave_direction(df, x_variable, y_variable, Coordinates, date_range, title, units, direc):
    #inputs
    #df : pd.DataFrame : the entire dataframe as it is passed through from the run_code script
    #x_variable: str : x variable name for the heatmap. like mwd
    #y_variable: ndarray : input array like swh. if hmax, alter the bin sizes
    #the titles are used in plotting
    ...
    x[x>345] = x[x > 345] - 360
    bins_y = np.linspace(np.floor(min(y)), np.ceil(max(y)), int(np.ceil(max(y)) - np.floor(min(y)))*2+1) #converts height into 0.5 bins
    bins_x = np.arange(-15, 361, 30) #returns bins with detla=30 bins
    counts, xedges, yedges = np.histogram2d(x, y, bins=(bins_x, bins_y))
    counts = counts/len(df) * 100
    counts = np.flip(counts, axis = 0)
    #following loops are important to create customized labels. 
...
def frequency_occurence_wind_direction(df, x_variable, y_variable, Coordinates, date_range, title, units, direc):
    #inputs
    #df : pd.DataFrame : the entire dataframe as it is passed through from the run_code script
    #x_variable: str : x variable name for the heatmap. like mwd
    #y_variable: ndarray : input array like swh
    ...
    bins_x = np.arange(-15, 361, 30) #returns bins with detla=1 bins
    counts, xedges, yedges = np.histogram2d(x, y, bins=(bins_x, bins_y))
    counts = counts/len(df) * 100
    counts = np.flip(counts, axis = 0)
    #following loops are important to create customized labels. 
