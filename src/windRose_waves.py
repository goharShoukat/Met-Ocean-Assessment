...
class WindroseAxes(PolarAxes):
    ...
    def legend(self, loc="lower left", decimal_places=1, units=None, **kwargs):
        ...
        def get_handles():
            ...
            handles.insert(0,  mpl.patches.Rectangle((0, 0), 0.2, 0.2, facecolor='white', edgecolor="black" ))
            return handles
        ...
        def get_labels(decimal_places=1, units=None):
            ...
            labels[len(labels) -1] = '>= 6 ({} %)'.format(self.percent_swh_above_6)
            labels.insert(0, '< 1 ({} %)'.format(self.percent_swh_below_1))
            return labels
    ...
    def bar(self, direction, var, **kwargs):
        ...
        self.percent_swh_below_1 = round(len(var[var < 1]) / len(var) * 100, 2)
        self.percent_swh_above_6 = round(len(var[var > 6]) / len(var) * 100, 2)
